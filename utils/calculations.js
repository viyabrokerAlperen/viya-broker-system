...
export function calculateFullVoyage(shipLat, shipLng, loadPortName, loadGeo, dischPortName, dischGeo, specs, market, shipSpeed, userQty, userLoadRate, userDischRate) {
    ...
    // Laytime calculation
    const loadLaytime = qty / loadRate;
    const dischLaytime = qty / dischRate;

    // Port operations
    const loadTurnTime = 1.0;
    const dischTurnTime = 1.0;
    const loadAnchorTime = 0.5;
    ...
    // Weather delay
    const weatherDelay = seaDays * 0.05;
