...
<head>
    ...
    <title>VIYA BROKER | Golden Age V14</title>
...
<body>
    <div class="loader" id="loader"><div style="text-align: center;"><div class="spinner" style="margin: 0 auto 20px;"></div><div style="font-family: var(--font-tech); color: var(--neon-cyan); font-size:1.2rem;">CALCULATING BEST ROUTE...</div></div></div>
...
// =================================================================
// 2. BACKEND (SADE VE GÜÇLÜ MANTIK)
// =================================================================
...
} catch (error) { console.error("Ports missing"); }
...
// --- EFSANE V14 NETWORK (YALIN VE GÜÇLÜ) ---
const SEA_NETWORK = {
    // Sadece Boğazlar ve Kritik Dönüşler
    ...
    AEGEAN_EXIT: [[25.8, 39.5], [25.0, 38.0], [23.5, 36.0]], // Sinop yok, Girit yok
...
// --- V14 ROTA MANTIĞI (SİNOP YOK, KARIŞIKLIK YOK) ---
function getSmartRoute(startPort, endPort) {
    ...
    // Basit Coğrafi Tanımlar
    const isBlackSea = (p) => p.lng > 27 && p.lat > 41.5; // Boğaz'ın kuzeyi
    const isMarmara = (p) => p.lng > 27 && p.lng < 30 && p.lat > 40 && p.lat < 41.5;
    const isAmericas = (p) => p.lng < -30;
    const isAsia = (p) => p.lng > 60;
    const isEurope = (p) => p.lat > 45 && p.lng > -10;

    // 1. Karadeniz/Marmara Çıkışı
    if (isBlackSea(startPort)) {
        ...
        // Marmara (örn: İstanbul) -> Çanakkale -> Ege
        // Burası kritik: Sinop'a gitmez, direkt güneye iner.
        path = path.concat(SEA_NETWORK.DARDANELLES, SEA_NETWORK.AEGEAN_EXIT);
    }

    // 2. Okyanus Geçişleri
    if (isAmericas(endPort)) {
        // Eğer Ege'den veya Karadeniz'den geliyorsak Cebelitarık'a git
        if (startPort.lng > 0 && !isEurope(startPort)) {
            path = path.concat(SEA_NETWORK.GIBRALTAR);
        }
        
        // Panama Kanalı Kontrolü (Basit Mantık)
        if (endPort.lng < -80 && endPort.lat < 20) { // Pasifik tarafı
    ...
    else if (isAsia(endPort)) {
        // Akdeniz içindeysek Süveyş'e git
        if (startPort.lng < 40 && !isEurope(startPort)) {
...
function generateAIAnalysis(profit, routeDesc, duration, revenue, vType) {
    ...
    return `<strong>AI STRATEGY (${vType}):</strong><br>Route: ${routeDesc} (${duration.toFixed(1)} days).<br>Status: ${margin > 10 ? 'HIGH PROFIT' : 'STANDARD'}`;
}
...
function findOpportunities(shipPosName, region, vType) {
    ...
    for(let i=0; i<6; i++) {
        ...
        const fuelCost = (route.dist / (specs.speed * 24)) * specs.cons * MARKET_DATA.vlsfo; 
        const opex = (route.dist / (specs.speed * 24)) * specs.opex;
...
app.listen(port, () => console.log(`VIYA BROKER V14 (GOLDEN AGE) running on port ${port}`));
